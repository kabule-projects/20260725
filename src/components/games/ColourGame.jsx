...
const ColourGame = ({ config, onComplete }) => {
  ...
  }, [currentStep, config.steps, onComplete]);

  return (
    <div className="relative w-full max-w-sm mx-auto bg-memory-dark/50 rounded-lg surreal-border p-4 select-none">
      <div className="relative w-full aspect-[4/3] bg-gradient-to-b from-gray-800 to-gray-900 rounded-lg overflow-hidden mb-4">
        {config.baseImage ? (
          <img 
        ...
        {config.steps.slice(0, currentStep + 1).map((step, index) => (
          step.layer ? (
            <img 
              key={index}
              src={step.layer}
      ...
      <AnimatePresence>
        ...
        {completed && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <p className="text-memory-glow text-lg mb-2">记忆完整了!</p>
            <p className="text-memory-muted text-sm mb-4">画面中的色彩都被找到</p>
            <motion.button
              className="px-6 py-2 bg-memory-glow/20 text-memory-glow rounded-lg border border-memory-glow/30"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
